...
from .physical_robot_model import PhysicalRobotModel
...
class RobotAgentNode(Node):
    def __init__(self):
        ...
        # Initialize physical robot model with proper dt
        initial_state = np.array([[2.0], [2.0], [0.0], [0.0], [0.0], [0.0]], dtype=np.float32)
        self.robot_model = PhysicalRobotModel(id=self.agent_id, x_ini=initial_state, dt=0.025)  # dt/4 for sub-steps
        ...
        self.get_logger().info(f'Robot Agent Node {self.agent_id} started and waiting for reset.')
    ...
    def reset_callback(self, msg):
        ...
        if initial_state.size == 6:
            ...
            # Convert ROS state to robot model format and reset physical model
            robot_state = self._convert_ros_to_robot_state(initial_state)
            self.robot_model.x = robot_state
    ...
    def action_callback(self, msg):
        ...
        # Convert normalized action to physical action format expected by PhysicalRobotModel
        physical_action = self._convert_action_to_physical(action)

        # Use physical model to evolve the system
        self.robot_model.evolve(physical_action)

        # Convert robot model state back to ROS format
        self.state = self._convert_robot_to_ros_state(self.robot_model.x)
    ...
    def _convert_ros_to_robot_state(self, ros_state):
        """Convert ROS state [x, y, yaw, vx, vy, vyaw] to robot state [x, y, psi, u, v, r]"""
    ...
    def _convert_robot_to_ros_state(self, robot_state):
        """Convert robot state [x, y, psi, u, v, r] to ROS state [x, y, yaw, vx, vy, vyaw]"""
        x, y, psi, u, v, r = robot_state.flatten()
    ...
    def _convert_action_to_physical(self, normalized_action):
        """Convert normalized action [-1,1] to physical forces for PhysicalRobotModel"""
        # normalized_action[0] -> yaw moment, normalized_action[1] -> surge force
        # Scale to match the ranges expected by PhysicalRobotModel.get_force_tau()
