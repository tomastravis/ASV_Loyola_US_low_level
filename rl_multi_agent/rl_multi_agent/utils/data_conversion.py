...
class DataConverter:
    ...
    def scale_action_to_physical(action_normalized, action_type):
        ...
        if action_type == "vyaw":
            ...
            return action_normalized * 0.5 * 2.3  # Scale to [-1.15, 1.15] N⋅m for robot model
        elif action_type == "surge_force":
            return action_normalized * 2.0  # Scale to [0, 2] N for robot model (after +1 offset)
    ...
    def scale_physical_to_normalized(action_physical, action_type):
        ...
        if action_type == "vyaw":
            ...
            return action_physical / (0.5 * 2.3)  # Reverse scaling for robot model
        elif action_type == "surge_force":
            return (action_physical - 1.0) / 2.0  # Reverse scaling for robot model (remove +1 offset)
